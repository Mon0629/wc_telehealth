...
function resolveRoomLink(item: DoctorAppointmentApiItem): string | null {
  return item.daily_room_url ?? item.room_link ?? item.meeting_link ?? null;
}
...
>()((set) => ({
  ...
  fetchDoctorAppointments: async (page = 1, limit = 10) => {
  ...
  fetchPatientAppointments: async (page = 1, limit = 10) => {
