...
export default function DoctorDashboard() {
  ...
  const doctorAppointments = useAppointmentStore((s) => s.doctorAppointments)
  const isLoading = useAppointmentStore((s) => s.isLoadingAppointments)
  const fetchDoctorAppointments = useAppointmentStore((s) => s.fetchDoctorAppointments)
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (cancelled) return
      try {
        await fetchDoctorAppointments(1, APPOINTMENTS_LIST_MAX_LIMIT)
      } catch {
  ...
  const displayName = [user?.firstName, user?.lastName].filter(Boolean).join(" ") || "Doctor"
